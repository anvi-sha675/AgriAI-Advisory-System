...
export function EmptyState({ icon: Icon, title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-14 px-6">
      ...
      <h3 className="text-base font-semibold text-ink dark:text-gray-100">{title}</h3>
...
const alertStyles = {
  ...
  success: "bg-secondary-50 border-secondary-200 text-primary-800 dark:bg-secondary-950/40 dark:border-secondary-800 dark:text-secondary-300",
  warning: "bg-earth-50 border-earth-200 text-earth-600 dark:bg-earth-950/30 dark:border-earth-800 dark:text-earth-400",
  error: "bg-red-50 border-red-200 text-red-700 dark:bg-red-950/40 dark:border-red-800 dark:text-red-300",
};

const alertIcons = { info: Info, success: CheckCircle2, warning: AlertTriangle, error: XCircle };

export function Alert({ type = "info", title, children, className = "" }) {
  const Icon = alertIcons[type];
  return (
    <div className={cn("flex items-start gap-3 rounded-xl border px-4 py-3", alertStyles[type], className)} role="alert">
